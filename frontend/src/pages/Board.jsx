...
const Board = () => {
  ...
  const ensureDefaultTimeLists = async (existingLists) => {
    ...
    for (const title of missingTitles) {
      await axios.post('https://trellooo.onrender.com/lists', {
        title,
        board_id: id,
  ...
  const fetchLists = async () => {
    try {
      const response = await axios.get(`https://trellooo.onrender.com/boards/${id}/lists`);
      const fetchedLists = response.data;
      await ensureDefaultTimeLists([...fetchedLists]);

      const refreshedResponse = await axios.get(`https://trellooo.onrender.com/boards/${id}/lists`);
  ...
  const handleAddCard = async (listId) => {
    ...
    try {
      ...
      await axios.post('https://trellooo.onrender.com/cards', { title: newCardTitle, list_id: listId, position, user_id: user?.id });
      setNewCardTitle('');
  ...
  const handleAddList = async () => {
    ...
    try {
      ...
      await axios.post('https://trellooo.onrender.com/lists', { title: newListTitle, board_id: id, position: pos });
      setNewListTitle('');
  ...
  const handleDeleteCard = async (e, cardId) => {
    ...
    try { await axios.delete(`https://trellooo.onrender.com/cards/${cardId}`); fetchLists(); } catch (err) { alert("Failed."); }
  };
  ...
  const fetchChecklist = async (cardId) => {
    try {
      ...
      const res = await axios.get(`https://trellooo.onrender.com/cards/${cardId}/checklist`);
  ...
  const handleAddChecklistItem = async () => {
    ...
    try {
      await axios.post('https://trellooo.onrender.com/checklist/item', {
  ...
  const handleToggleChecklistItem = async (itemId) => {
    try {
      await axios.put(`https://trellooo.onrender.com/checklist/item/${itemId}`);
  ...
  const handleDeleteChecklistItem = async (itemId) => {
    try {
      await axios.delete(`https://trellooo.onrender.com/checklist/item/${itemId}`);
      if (modalCard) fetchChecklist(modalCard.id);
    } catch (err) {
  ...
  const handleUpdateCard = async () => {
    ...
    try {
      await axios.put(`https://trellooo.onrender.com/cards/${modalCard.id}`, { 
        title: editCardTitle, 
  ...
  const handleDeleteList = async (listId) => {
    ...
    try { await axios.delete(`https://trellooo.onrender.com/lists/${listId}`); fetchLists(); } catch (err) { alert("Failed."); }
  };
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      await axios.put(`https://trellooo.onrender.com/cards/${activeId}/move`, { list_id: targetListId, position: 1 });
